...
After an undo, the function returns redo data, which can be used to perform redo
later. Redo is basically undo's undo. After redo, yet another undo data is
returned, ad nauseam.
...
according to the current state. To perform undo/redo, we are given undo/redo
