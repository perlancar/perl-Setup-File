...
test_tx_action(
    ...
    after_do    => sub {
        ok((-f "p"), "p is a file");
        is(scalar(read_text "p"), "", "p's content is empty");
    },
);
...
for my $existed (0, 1) {
    test_tx_action(
        ...
        after_do    => sub {
            ok((-f "p"), "p is a file");
            is(scalar(read_text "p"), "bar", "p's content is fixed");
        },
        after_undo  => sub {
            if ($existed) {
                ok((-f "p"), "p is a file");
                is(scalar(read_text "p"), "foo", "p's old content is restored");
            } else {
    ...
    test_tx_action(
        ...
        after_do    => sub {
            ok((-f "p"), "p is a file");
            is(scalar(read_text "p"), "", "p's content is fixed");
        },
        after_undo  => sub {
            if ($existed) {
                ok((-f "p"), "p is a file");
